    ...
    <section className="bg-white">
      <div className="max-w-7xl mx-auto px-6 py-20">
        ...
        <div className="mt-10 grid grid-cols-1 lg:grid-cols-3 gap-6">
          {tiers.map((tier, i) => (
            <Reveal key={tier.name} direction={i === 0 ? 'left' : i === 1 ? 'up' : 'right'}>
              <div className={`rounded-2xl border ${tier.highlight ? 'border-emerald-300 ring-2 ring-emerald-200' : 'border-slate-200'} bg-white p-6 shadow-sm`}>                
                {tier.highlight && (
